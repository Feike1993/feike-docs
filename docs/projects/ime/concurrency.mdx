...
4. 处理异常时把消息 `put` 回队列再试（`retryLimit = 3` 被 `put` 成功后的 `break` 短路，实际只重入一次、没有按消息维度的计数）；异常/中断时 sleep 100ms 防空转
5. 每 1000 条打一次日志，用 `remainingCapacity()` 而不是 `queue.size()`
6. `item_value` 超 1000 字符时截取前 1000 再拼 `"... (truncated)"`（最终约 1015 字符）；仅 `DeviceItemListener` 生效，`MqttDeviceDataSubscribeListener.sendData` 不截断
...
`CallerRunsPolicy` 在池满时把任务打回调用线程，形成背压，而不是丢消息。Kafka 生产端另有 `linger.ms`（测试 200ms，5 个 prod profile 全部 2000ms）、`batch-size` 16KB、`acks=1`、`retries=10`。

## 双 Kafka：最新态 + 追加日志

`sendData(topic, key, data)` 单发一个 Topic；双发在调用方 `sendDataToKafka` 里对同一 JSON 连续发 `device_item_uni` 与 `device_item_dup`，分区 key 一般是 `deviceCode`：
...
但 `$queue/device_item_utf_8` 在这个类上的 `@MqttClientSubscribe` **已注释**，定时 `processBatchIfNeeded` 未启动，连 `adjustBatchSize()` 的调用点（在 `processBatchAsync` 内）也被注释——整套批次引擎实际是**死代码**。而且初始 `batchSize = new AtomicInteger(1000)`，本身就超出 `MIN=1 / MAX=100` 的范围。需要分清的是：这是按耗时在吞吐和延迟之间调批次的设计，**线上主流量改成了「无界队列 + 单消费线程 + 线程池扇出」**，避免回调线程和批次窗口互相卡住。
...
主路径是「无界队列 + 单消费线程 + 池化扇出」，吞吐好但有明确的取舍：

- **队列无上限**：`LinkedBlockingQueue` 不设容量，真正刹住车的是线程池 `CallerRunsPolicy`；但背压只在池满时触发，队列本身会先积累，内存压力取决于下游多快恢复。
- **单线程 drain 是单点**：`take` + 解析补名都在一个线程，解析变慢会卡住整条链路。水平扩容靠加边端实例（共享订阅分摊），不是加 drain 线程。
- **`CallerRunsPolicy` 的双刃**：池满时任务打回调用线程（也就是 drain 线程）执行，能背压，但会让 drain 线程被 Kafka/落库阻塞，反过来拖慢消费。
