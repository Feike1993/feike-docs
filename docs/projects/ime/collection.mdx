...
| 工业网关 | `2` 网关 | `ReportRtData`、`DeviceConnectionStatus`、`ReportHistData` | `ReportRtData` 读 `pld.array[].datas` 并回 `ReportRtDataAck`；`DeviceConnectionStatus` 读 `pld.deviceIds[].deviceId/result`、不回 ACK |

KepServer 点位 `id` 按 `.` 拆：第 2 段（`idArray[1]`）是设备编码；第 3 段 `working` / `fault` 分别落到运行、故障；联网离线要第 3 段 `_System` + 第 4 段 `_Error` 同时命中。`_Error` 是「故障标志」，**true → 离线（status_value=1），false → 在线（0）**（代码里是 `value ? 1 : 0` 直接映射，不是取反）。
