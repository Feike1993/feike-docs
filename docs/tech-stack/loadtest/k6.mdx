...
它测的是 HTTP / WebSocket / gRPC 这类接口行为，不是浏览器里点按钮。和 [ai-example](https://github.com/Feike1993/ai-example) 的分工是：[Playwright 工业 E2E](/docs/tech-stack/e2e/playwright) 负责工业页点击与无模型探针，k6 负责打 Java 的 `/api/v1`。
...
对应仓库 [loadtest/](https://github.com/Feike1993/ai-example/tree/main/loadtest)，打的是本机 Java **`http://127.0.0.1:8080/ai-example/api/v1`**，不经过 Vite，也不走 Compose 前端的 8088。限流原理见 [Redis 令牌桶](/docs/ai-example/production/redis-token-bucket)。
